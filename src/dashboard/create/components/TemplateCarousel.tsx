...
const TemplateCarousel: React.FC<TemplateCarouselProps> = ({ templates, onSelect }) => {
    ...
    return (
        <div className="relative w-full h-[800px] flex items-center justify-center overflow-visible perspective-1000 py-10" ref={containerRef}>
            ...
            <div className={`absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary/20 blur-[100px] rounded-full transition-opacity duration-1000 z-0 ${activeIndex >= 0 ? 'opacity-100' : 'opacity-0'}`}></div>

            {/* Render Cards */}
            <div className="relative w-full h-full flex items-center justify-center transform-style-preserve-3d" style={{ transform: 'translateY(-50px)' }}> {/* Lift to center vertically better */}
                {templates.map((template, index) => {
                    ...
                    return (
                        <div
                            key={template.id}
                            className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-[450px]"
                            style={{
